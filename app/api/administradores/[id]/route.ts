...
// PATCH: actualizar administrador (nombre, correo, activo)
export async function PATCH(
  ...
  const { error } = await supa.from("administradores").update(patch).eq("id", params.id);

  if (error) {
    if (/duplicate|unique/i.test(error.message)) {
      return NextResponse.json(
        { error: "Ya existe otro administrador con ese correo" },
        { status: 409 }
      );
    }
